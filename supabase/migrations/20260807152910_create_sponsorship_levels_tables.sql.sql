/*
# Sponsorship Levels Module — New Tables

## Purpose
Adds a brand-new Sponsorship Levels system to the existing Donation Guide.
This is purely additive — no existing tables are modified.

## New Tables

1. `donation_sponsorship_levels`
   - Stores sponsorship tiers (Bronze, Silver, Gold, Platinum, etc.)
   - Fields: id, name_en, name_es, description_en, description_es,
     min_amount, max_amount, button_text_en, button_text_es,
     button_action (stripe | paypal | both | contact), icon, color,
     featured (bool), status (active | inactive), display_order,
     created_at, updated_at

2. `donation_sponsorship_benefits`
   - Stores individual benefits (each exists only once, no duplication)
   - Fields: id, text_en, text_es, display_order, created_at, updated_at

3. `donation_sponsorship_benefit_levels` (junction)
   - Links benefits to levels with an `included` boolean
   - Fields: id, benefit_id (FK), level_id (FK), included (bool)
   - Unique constraint on (benefit_id, level_id) to prevent duplicates

4. `donation_sponsorship_section`
   - Single-row table for the editable section header (title/subtitle/description in EN+ES)
   - Fields: id, title_en, title_es, subtitle_en, subtitle_es,
     description_en, description_es, updated_at

## Seed Data
- 4 sponsorship levels: Bronze, Silver, Gold, Platinum
- 10 common benefits
- Benefit-level junction rows matching a typical sponsorship comparison table
- 1 section header row with default EN/ES text

## Security
- RLS enabled on all 4 tables.
- Policies: `TO anon, authenticated` for SELECT (public read via API);
  `TO authenticated` for INSERT/UPDATE/DELETE (admin only).
  Note: The app uses Prisma via DATABASE_URL which bypasses RLS, so these
  policies are a defense-in-depth measure for any direct Supabase access.
*/
...
-- Bronze = benefits 0-3, Silver = 0-5, Gold = 0-7, Platinum = 0-9
...
-- Also create the "not included" junction rows so the matrix is complete
INSERT INTO donation_sponsorship_benefit_levels (benefit_id, level_id, included)
