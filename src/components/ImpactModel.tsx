  ...
  return (
    <div className="relative flex h-56 w-56 items-center justify-center sm:h-64 sm:w-64">
      <svg viewBox="0 0 200 200" className="h-full w-full -rotate-90">
        <circle cx="100" cy="100" r={radius} fill="none" stroke="#eceef2" strokeWidth="22" />
        {segments.map((s) => {
      ...
      <div className="absolute flex flex-col items-center">
        <span className="font-display text-3xl font-extrabold text-ink-900">$6K–8K</span>
        <span className="text-xs font-medium uppercase tracking-wider text-ink-400">{t.impact.perWork}</span>
      </div>
    </div>
  );
}
    ...
    <Section id="impacto" className="relative overflow-hidden bg-white">
      ...
      <Reveal className="max-w-3xl">
        <span className="eyebrow text-primary-700">
          <Handshake className="h-4 w-4" /> {t.impact.eyebrow}
        </span>
        <h2 className="mt-4 font-display text-3xl font-extrabold tracking-tight text-ink-900 sm:text-4xl lg:text-5xl text-balance">
          {t.impact.title1} <span className="text-gradient-blue">{t.impact.title2}</span> {t.impact.titleSuffix}
        </h2>
              ...
              <div className="rounded-2xl border border-ink-100 bg-ink-50/60 p-5">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <span className={`flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${d.color} text-white shadow-md`}>
                      <d.icon className="h-5 w-5" />
                    </span>
                    <div>
                      <p className="font-display text-base font-bold text-ink-900">{t.impact.distribution[i].label}</p>
                      <p className="text-xs text-ink-500">{t.impact.distribution[i].detail}</p>
                    </div>
                  </div>
                  <span className="font-display text-2xl font-extrabold text-ink-900">{d.pct}%</span>
                </div>
      ...
      <div className="mt-20 border-t border-ink-100 pt-14">
          ...
          {t.impact.sponsorTiers.map((tier, i) => (
            <Reveal key={tier.name} delay={(i + 1) as 1 | 2 | 3 | 4}>
              <div className="group h-full rounded-3xl border border-ink-100 bg-white p-6 transition-all duration-300 hover:-translate-y-1 hover:border-primary-200 hover:shadow-card">
                ...
                <p className="mt-4 font-display text-base font-bold text-ink-900">{tier.name}</p>
                <p className="mt-1 text-sm leading-relaxed text-ink-500">{tier.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delay={2} className="mt-10">
            ...
            <div className="flex w-max animate-marquee gap-12 px-6">
              {[...Array(2)].map((_, dup) => (
                <div key={dup} className="flex gap-12">
                  {['Corporate Partner', 'Strategic Co.', 'Media Platform', 'Community Org', 'Tech Ally', 'Foundation'].map((name) => (
                    <span key={name + dup} className="whitespace-nowrap font-display text-lg font-bold text-ink-300">
                      {name}
                    </span>
                  ))}
                </div>
          ...
          <p className="mt-3 text-center text-xs text-ink-400">{t.impact.logosNote}</p>
        </Reveal>
      </div>
    </Section>
  );
